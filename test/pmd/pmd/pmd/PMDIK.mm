...
void cPMDIK::update( void )
{
	...
	for( unsigned short it = 0 ; it < m_unCount ; it++ )
	{
		for( unsigned char cbLinkIdx = 0 ; cbLinkIdx < m_cbNumLink ; cbLinkIdx++ )
		{
			...
			Matrix	matInvBone;
			VecMatQuat::MatrixInverse( matInvBone, m_ppBoneList[cbLinkIdx]->m_matLocal );

			// エフェクタ，到達目標のローカル位置
			VecMatQuat::Vector3Transform( &vec3EffPos, &vec3EffPos, matInvBone );
			VecMatQuat::Vector3Transform( &vec3TargetPos, &vec3OrgTargetPos, matInvBone );

			// 十分近ければ終了
			Vector3	vec3Diff;
			VecMatQuat::Vector3Sub( &vec3Diff, &vec3EffPos, &vec3TargetPos );
			if( VecMatQuat::Vector3DotProduct( &vec3Diff, &vec3Diff ) < 0.0000001f )	return;

			// (1) 基準関節→エフェクタ位置への方向ベクトル
			VecMatQuat::Vector3Normalize( &vec3EffPos, &vec3EffPos );

			// (2) 基準関節→目標位置への方向ベクトル
			VecMatQuat::Vector3Normalize( &vec3TargetPos, &vec3TargetPos );

			// ベクトル (1) を (2) に一致させるための最短回転量（Axis-Angle）
			//
			// 回転角
			float	fRotAngle = acosf( VecMatQuat::Vector3DotProduct( &vec3EffPos, &vec3TargetPos ) );
			...
			if( 0.00000001f < fabsf( fRotAngle ) )
			{
				...
				VecMatQuat::Vector3CrossProduct( &vec3RotAxis, &vec3EffPos, &vec3TargetPos );
				if( VecMatQuat::Vector3DotProduct( &vec3RotAxis, &vec3RotAxis ) < 0.0000001f )	continue;

				VecMatQuat::Vector3Normalize( &vec3RotAxis, &vec3RotAxis );

				// 関節回転量の補正
				Vector4		vec4RotQuat;
				VecMatQuat::QuaternionCreateAxis( &vec4RotQuat, &vec3RotAxis, fRotAngle );

				if( m_ppBoneList[cbLinkIdx]->m_bIKLimitAngle )	limitAngle( &vec4RotQuat, &vec4RotQuat );

				VecMatQuat::QuaternionNormalize( &vec4RotQuat, &vec4RotQuat );

				VecMatQuat::QuaternionMultiply( &m_ppBoneList[cbLinkIdx]->m_vec4Rotation, &m_ppBoneList[cbLinkIdx]->m_vec4Rotation, &vec4RotQuat );
				VecMatQuat::QuaternionNormalize( &m_ppBoneList[cbLinkIdx]->m_vec4Rotation, &m_ppBoneList[cbLinkIdx]->m_vec4Rotation );
...
void cPMDIK::limitAngle( Vector4 *pvec4Out, const Vector4 *pvec4Src )
{
	...
	// XYZ軸回転の取得
	VecMatQuat::QuaternionToEuler( &vec3Angle, pvec4Src );
	...
	// XYZ軸回転からクォータニオンへ
	VecMatQuat::QuaternionCreateEuler( pvec4Out, &vec3Angle );
}
