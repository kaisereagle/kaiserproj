...
void cPMDBone::initialize( const PMD_Bone *pPMDBoneData, const cPMDBone pBoneArray[] )
{
	...
	if( pPMDBoneData->nParentNo != -1 )
	{
		...
		VecMatQuat::Vector3Sub( &m_vec3Offset, &m_vec3OrgPosition, &m_pParentBone->m_vec3OrgPosition );
	}
	else
	{
		// 親なし
	...
	VecMatQuat::MatrixIdentity(	m_matInvTransform );
...
{
	if( m_pParentBone )	VecMatQuat::Vector3Sub( &m_vec3Offset, &m_vec3OrgPosition, &m_pParentBone->m_vec3OrgPosition );
}
...
void cPMDBone::reset( void )
{
	...
	VecMatQuat::MatrixIdentity(	m_matLocal );
...
void cPMDBone::updateMatrix( void )
{
	...
	VecMatQuat::QuaternionToMatrix( m_matLocal, &m_vec4Rotation );
	...
	// 親があるなら親の回転を受け継ぐ
	if( m_pParentBone )	VecMatQuat::MatrixMultiply( m_matLocal, m_matLocal, m_pParentBone->m_matLocal );
}
...
void cPMDBone::lookAt( const Vector3 *pvecTargetPos, float fLimitXD, float fLimitXU, float fLimitY  )
{
	...
	VecMatQuat::MatrixIdentity(	matTemp );
	...
	{
		Matrix	matInvTemp;
		VecMatQuat::MatrixInverse( matInvTemp, m_pParentBone->m_matLocal );
		...
		VecMatQuat::MatrixMultiply( matTemp, matTemp, matInvTemp );
	}
	VecMatQuat::MatrixInverse( matTemp, matTemp );

	Vector3		vec3LocalTgtPosZY;
	Vector3		vec3LocalTgtPosXZ;

	VecMatQuat::Vector3Transform( &vec3LocalTgtPosZY, pvecTargetPos, matTemp );

	vec3LocalTgtPosXZ = vec3LocalTgtPosZY;
	vec3LocalTgtPosXZ.y = 0.0f;
	VecMatQuat::Vector3Normalize( &vec3LocalTgtPosXZ, &vec3LocalTgtPosXZ );

	vec3LocalTgtPosZY.x = 0.0f;
	VecMatQuat::Vector3Normalize( &vec3LocalTgtPosZY, &vec3LocalTgtPosZY );
	...
	VecMatQuat::QuaternionCreateEuler( &vec4RotTemp, &vec3Angle );
	VecMatQuat::QuaternionSlerp( &m_vec4LookRotation, &m_vec4LookRotation, &vec4RotTemp, 0.3f );
...
{
	VecMatQuat::MatrixMultiply( m_matSkinning, m_matInvTransform, m_matLocal );
}
