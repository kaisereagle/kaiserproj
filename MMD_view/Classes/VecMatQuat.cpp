...
void VecMatQuat::vec_add( vec3 *pvec3Out, const vec3 *pvec3Add1, const vec3 *pvec3Add2 )
{
...
void VecMatQuat::vec_sub( vec3 *pvec3Out, const vec3 *pvec3Sub1, const vec3 *pvec3Sub2 )
{
...
void VecMatQuat::vec_add_mul( vec3 *pvec3Out, const vec3 *pvec3Add1, const vec3 *pvec3Add2, float fRate )
{
...
void VecMatQuat::vec_nomalize( vec3 *pvec3Out, const vec3 *pvec3Src )
{
...
float VecMatQuat::vec_dot_product( const vec3 *pvec3Src1, const vec3 *pvec3Src2 )
{
	return (pvec3Src1->x * pvec3Src2->x + pvec3Src1->y * pvec3Src2->y + pvec3Src1->z * pvec3Src2->z);
}

void VecMatQuat::vec_cross_product( vec3 *pvec3Out, const vec3 *pvec3Src1, const vec3 *pvec3Src2 )
{
...
void VecMatQuat::vec_lerp( vec3 *pvec3Out, const vec3 *pvec3Src1, const vec3 *pvec3Src2, float fLerpValue )
{
...
void VecMatQuat::vec_transform( vec3 *pvec3Out, const vec3 *pVec3In, const matrix matTransform )
{
	vec3	vec3Temp;
...
void VecMatQuat::vec_rotate( vec3 *pvec3Out, const vec3 *pVec3In, const matrix matRotate )
{
	vec3	vec3Temp;
...
void VecMatQuat::mat_identity( matrix matOut )
{
...
void VecMatQuat::mat_rotation( matrix matOut, float fAngle )
{
	float	c = cosf( fAngle );
	float	s = sinf( fAngle );
...
void VecMatQuat::mat_mul( matrix matOut, const matrix matSrc1, const matrix matSrc2 )
{
	matrix	matTemp;
	int		i;
...
void VecMatQuat::mat_inverse( matrix matOut, const matrix matSrc )
{
	matrix	matTemp;
...
void VecMatQuat::mat_lerp( matrix matOut, matrix matSrc1, matrix matSrc2, float fLerpValue )
{
...
void mat_planar_projection( matrix matOut, const vec4 *pvec4Plane, const vec3 *pvec3LightPos )
{
...
void VecMatQuat::quat_create_axis( vec4 *pvec4Out, const vec3 *pvec3Axis, float fRotAngle )
{
...
void VecMatQuat::quat_create_euler( vec4 *pvec4Out, const vec3 *pvec3EulerAngle )
{
...
void VecMatQuat::quat_nomalize( vec4 *pvec4Out, const vec4 *pvec4Src )
{
...
void VecMatQuat::quat_mul( vec4 *pvec4Out, const vec4 *pvec4Src1, const vec4 *pvec4Src2 )
{
	float	px, py, pz, pw;
	float	qx, qy, qz, qw;
...
void VecMatQuat::quat_slerp( vec4 *pvec4Out, const vec4 *pvec4Src1, const vec4 *pvec4Src2, float fLerpValue )
{
#if	0
	// Slerp
	...
	quat_nomalize( pvec4Out, pvec4Out );
#endif
}

void VecMatQuat::quat_to_mat( matrix matOut, const vec4 *pvec4Quat )
{
...
void VecMatQuat::quat_to_euler( vec3 *pvecAngle, const vec4 *pvec4Quat )
{
	// XYZ軸回転の取得
	// Y回転を求める
