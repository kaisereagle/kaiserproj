...
void cMotionPlayer::setup( cPMDModel *pPMDModel, cVMDMotion *pMotion, bool bLoop, float fInterpolateFrame )
{
	...
	{
		(*ppBone) = pPMDModel->getBoneByName_( pMotionDataList->szBoneName );
	...
	else
	{
		...
		while( pMotionDataList )
		{
			if( *ppBone )
			{
				...
				VecMatQuat::vec_lerp( &((*ppBone)->m_vec3Position), &((*ppBone)->m_vec3Position), &vec3Position, fInterpolateRate );
				VecMatQuat::quat_slerp( &((*ppBone)->m_vec4Rotation), &((*ppBone)->m_vec4Rotation), &vec4Rotation, fInterpolateRate );
			}
...
// キーフレームを補間して位置と回転を返す
//----------------------------------------
void cMotionPlayer::getMotionPosRot( const MotionDataList *pMotionData, float fFrame, vec3 *pvec3Pos, vec4 *pvec4Rot )
{
	unsigned long	i;
	...
	if( lKey0 != lKey1 )
	{
		...
		VecMatQuat::quat_slerp( pvec4Rot, &(pMotionData->pKeyFrames[lKey0].vec4Rotation), &(pMotionData->pKeyFrames[lKey1].vec4Rotation), fRotLerpValue );
	}
	else
	{
