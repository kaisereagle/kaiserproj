...
typedef struct _Vertex {
 ...
 void __log(char *in_p,int in_num)
{
    NSString *str = [ [ NSString alloc ] initWithUTF8String:in_p ];
    NSLog(str,in_num);
}
...
static void matrixPlanarProjection( float matOut[4][4], const vec4 *pvec4Plane, const vec3 *pvec3LightPos )
{
...
- (id) init
{
	if (self = [super init])
	{
		...
		NSString *path2;	
		NSError *error;
        path2 = [[NSBundle mainBundle] pathForResource:@"01" ofType:@"mp3"];
		...
		g_clPMDModel.load(MMD_MODEL);
		g_clVMDMotion.load("1.vmd");
       ...
       g_clPMDModel.resetRigidBodyPos();
		...
		// 陰付け処理
		vec4		vec4Plane = { 0.0f, 1.0f, 0.0f, 0.0f };		// { a, b, c, d } ax + by + cz + d = 0
		vec3		vec4LightPos = { 10.0f, 70.0f, -20.0f };	// 光源
...
- (void)drawScene:(GLfloat)screenWidth with:(GLfloat) screenHeight anglex:(float) anglex angley:(float)angley distance:(float) distance {
    ...
    g_clBulletPhysics.update(temp-priviousTime);
	priviousTime = temp;
	//GlTrans te;
