...
point+= ulSize;
   ...
   // NSLog(@"... PMDModel::m_ulNumMaterials = [%d]", m_ulNumMaterials);
    ...
    __log("mat num:%d",m_ulNumMaterials);
	...
	for( unsigned short i = 0 ; i < m_ulNumMaterials ; i++ )
	{
        ...
        __log("i:%d",i);
        __log(pPMDMaterial->szTextureFileName,0);
		pPMDMaterial++;
	}
...
//    m_unNumBones = 174;
    ...
    __log(" m_numBone = [%d]", m_unNumBones);
    char sjisBuff[21];
    char *m_name;
	// ボーン配列を作成
	...
	for( unsigned short i = 0 ; i < m_unNumBones ; i++ )
	{
        ...
        strncpy(sjisBuff, m_pBoneArray[i].getName(), 20);
        sjisBuff[20] = '\0';
        m_name=MMDFiles_strdup_from_sjis_to_utf8(sjisBuff);
		// 首のボーンを保存
		if( strncmp( sjisBuff, "頭", 20 ) == 0 )	m_pNeckBone = &m_pBoneArray[i];
...
cPMDBone *cPMDModel::getBoneByName( const char *szBoneName )
{
	...
	{
		if( strncmp( pBoneArray->getName(), MMDFiles_strdup_from_utf8_to_sjis(szBoneName), 20 ) == 0 )
			return pBoneArray;
	}
...
cPMDFace *cPMDModel::getFaceByName( const char *szFaceName )
{
	...
	{
		if( strncmp( pFaceArray->getName(), MMDFiles_strdup_from_utf8_to_sjis(szFaceName), 20 ) == 0 )
			return pFaceArray;
	}
...
//
	...
	for( unsigned long i = 0 ; i < m_ulNumVertices ; i++ )
	{
		...
		{
			VecMatQuat::Vector3Transform( &m_pvec3PositionArray[i], &m_pvec3OrgPositionArray[i], m_pBoneArray[m_pOrgSkinInfoArray[i].unBoneNo[1]].m_matSkinning );
			VecMatQuat::Vector3Rotate( &m_pvec3NormalArray[i], &m_pvec3OrgNormalArray[i], m_pBoneArray[m_pOrgSkinInfoArray[i].unBoneNo[1]].m_matSkinning );
		}
		else if( m_pOrgSkinInfoArray[i].fWeight >= 0.9999f )
		{
			VecMatQuat::Vector3Transform( &m_pvec3PositionArray[i], &m_pvec3OrgPositionArray[i], m_pBoneArray[m_pOrgSkinInfoArray[i].unBoneNo[0]].m_matSkinning );
			VecMatQuat::Vector3Rotate( &m_pvec3NormalArray[i], &m_pvec3OrgNormalArray[i], m_pBoneArray[m_pOrgSkinInfoArray[i].unBoneNo[0]].m_matSkinning );
		}
		else
		{
			VecMatQuat::MatrixLerp( matTemp,	m_pBoneArray[m_pOrgSkinInfoArray[i].unBoneNo[0]].m_matSkinning,
									m_pBoneArray[m_pOrgSkinInfoArray[i].unBoneNo[1]].m_matSkinning,		m_pOrgSkinInfoArray[i].fWeight );

			VecMatQuat::Vector3Transform( &m_pvec3PositionArray[i], &m_pvec3OrgPositionArray[i], matTemp );
			VecMatQuat::Vector3Rotate( &m_pvec3NormalArray[i], &m_pvec3OrgNormalArray[i], matTemp );
		}
	}
}
