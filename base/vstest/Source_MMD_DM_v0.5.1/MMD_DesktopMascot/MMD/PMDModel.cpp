...
//--------------------------
//// 色のRGBどれかが1を超えていた場合、ノーマライズを行う
////--------------------------
//static void normalizeColor( float *pfCol )
//{
//	if (pfCol[3] > 1.0f) pfCol[3] = 1.0f;		// α値はRGBとは別に、最大1に揃える
//
//	float len = 0.0f;
...
//
//	if (len == 0.0f) return;	// RGBいずれも1を超えていなければ、そのまま終了
//
//	pfCol[0] /= len;
//	pfCol[1] /= len;
//	pfCol[2] /= len;
...
bool cPMDModel::initialize( unsigned char *pData, unsigned long ulDataSize )
{
	...
	for( unsigned short i = 0 ; i < m_unNumBones ; i++ )
	{
		...
		// センターのボーンを保存
		if( strncmp( m_pBoneArray[i].getName(), "センター", 20 ) == 0 ) m_pCenterBone = &m_pBoneArray[i];
