...
bool cBulletPhysics::initialize( void )
{
	...
	//m_pBtWorld->setGravity( btVector3( 0.0f, -9.8f, 0.0f ) );
...
void cBulletPhysics::removeFromWorld( btRigidBody *pbtRB )
{
	...
	m_pBtWorld->removeRigidBody( pbtRB );
}
...
void cBulletPhysics::update( float fElapsedFlame )
{
	...
	// 2009/08/04 Ru--en	確信は持てないけど修正
	float fSec = fElapsedFlame / 30.0f;
	int   nMaxSubStep = 1;					// 最大分割数らしいので適当に与えてみる
	//m_pBtWorld->stepSimulation( fSec, nMaxSubStep, 1.0f / 30.0f );
	m_pBtWorld->stepSimulation( fSec, nMaxSubStep );
}
