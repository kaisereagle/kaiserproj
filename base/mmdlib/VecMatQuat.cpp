...
void vec_add( vec3 *out_vec, const vec3 *in_add1, const vec3 *in_add2 )
{
	out_vec->x = in_add1->x + in_add2->x;
	out_vec->y = in_add1->y + in_add2->y;
	out_vec->z = in_add1->z + in_add2->z;
}

void vec_sub( vec3 *out_vec, const vec3 *in_base, const vec3 *in_sub )
{
	out_vec->x = in_base->x - in_sub->x;
	out_vec->y = in_base->y - in_sub->y;
	out_vec->z = in_base->z - in_sub->z;
}

void vec_add_mul( vec3 *pvec3Out, const vec3 *pvec3Add1, const vec3 *pvec3Add2, float fRate )
{
...
void vec_nomalize( vec3 *pvec3Out, const vec3 *pvec3Src )
{
...
float vec_dot_product( const vec3 *pvec3Src1, const vec3 *pvec3Src2 )
{
	return (pvec3Src1->x * pvec3Src2->x + pvec3Src1->y * pvec3Src2->y + pvec3Src1->z * pvec3Src2->z);
}

void vec_cross_product( vec3 *pvec3Out, const vec3 *pvec3Src1, const vec3 *pvec3Src2 )
{
...
void vec_lerp( vec3 *pvec3Out, const vec3 *pvec3Src1, const vec3 *pvec3Src2, float fLerpValue )
{
...
void vec_transform( vec3 *pvec3Out, const vec3 *pVec3In, const mtrx matTransform )
{
	vec3	vec3Temp;
...
void vec_rotate( vec3 *pvec3Out, const vec3 *pVec3In, const mtrx matRotate )
{
	vec3	vec3Temp;
...
void mat_rotation( mtrx matOut, float fAngle )
{
	float	c = cosf( fAngle );
	float	s = sinf( fAngle );
...
void mat_mul( mtrx matOut, const mtrx matSrc1, const mtrx matSrc2 )
{
	mtrx	matTemp;
	int		i;
...
void mat_inverse( mtrx matOut, const mtrx matSrc )
{
	mtrx	matTemp;
	memcpy( matTemp, matSrc, sizeof(mtrx) );
...
void mat_lerp( mtrx matOut, mtrx matSrc1, mtrx matSrc2, float fLerpValue )
{
...
void mat_planar_projection( mtrx matOut, const vec4 *pvec4Plane, const vec3 *pvec3LightPos )
{
...
void quat_create_axis( vec4 *pvec4Out, const vec3 *pvec3Axis, float fRotAngle )
{
...
void quat_create_euler( vec4 *pvec4Out, const vec3 *pvec3EulerAngle )
{
...
void quat_nomalize( vec4 *pvec4Out, const vec4 *pvec4Src )
{
...
void quat_mul( vec4 *pvec4Out, const vec4 *pvec4Src1, const vec4 *pvec4Src2 )
{
	float	px, py, pz, pw;
	float	qx, qy, qz, qw;
...
void quat_slerp( vec4 *pvec4Out, const vec4 *pvec4Src1, const vec4 *pvec4Src2, float fLerpValue )
{
#if	0
	// Slerp
...
void quat_to_mat( mtrx matOut, const vec4 *pvec4Quat )
{
...
void quat_to_euler( vec3 *pvecAngle, const vec4 *pvec4Quat )
{
	// XYZ軸回転の取得
	// Y回転を求める
