	...
	else
	{
		...
		while( pMotionDataList )
		{
			if( *ppBone )
			{
				...
				vec_lerp( &((*ppBone)->m_vec3Position), &((*ppBone)->m_vec3Position), &vec3Position, fInterpolateRate );
				quat_slerp( &((*ppBone)->m_vec4Rotation), &((*ppBone)->m_vec4Rotation), &vec4Rotation, fInterpolateRate );
			}
...
void cMotionPlayer::getMotionPosRot( const MotionDataList *pMotionData, float fFrame, vec3 *pvec3Pos, vec4 *pvec4Rot )
{
	...
	if( lKey0 != lKey1 )
	{
		...
		quat_slerp( pvec4Rot, &(pMotionData->pKeyFrames[lKey0].vec4Rotation), &(pMotionData->pKeyFrames[lKey1].vec4Rotation), fRotLerpValue );
	}
	else
	{
