...
- (void)viewDidLoad {
    ...
    char *path;
    pathCFURLRef = CFBundleCopyResourcesDirectoryURL(CFBundleGetMainBundle());
    // Get relative path to resources directory.
    if (pathCFURLRef) {
        path = (char *)calloc(MAXPATHLEN, sizeof(char));
    ...
    NSString *imageFile = [bundlePath stringByAppendingPathComponent:@"alice.pmd"];
    if (!pmdModel->Load("alice.pmd", "/home"))
    {
