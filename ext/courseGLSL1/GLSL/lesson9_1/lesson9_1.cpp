...
#define VSHFILENAME	"..\\shader\\sin.vs"
#define FSHFILENAME	"..\\shader\\sin.fs"
...
DrawObject()
{
	...
	static ATTRIBUTE attribute[640];
	int n;
	float l;

	for (n = 0, l = -300.0f; l <= 300.0f; n++, l += 1.0f) {
		attribute[n].vertex[0] = l;
		attribute[n].vertex[1] = 0.0f;
		attribute[n].vertex[2] = 0.0f;
		attribute[n].startTime = (float)n;
	}
