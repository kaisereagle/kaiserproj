...
//	}
	...
	ConvString( ( const char * )FullPath, WCHAR_T_CHARCODEFORMAT, utf8_PathBuffer, utf8_PathBufferBytes, DX_CHARCODEFORMAT_UTF8 ) ;
...
extern int ReadOnlyFileAccessFindNext_PF(	FINDINFO *FindInfo, FILEINFOW *Buffer )
{
	...
	if( FindInfo->PF.Dir != NULL )
	{
		for(;;)
		{
			...
			ConvString( ret->d_name, DX_CHARCODEFORMAT_UTF8, ( char * )FileNameW, sizeof( FileNameW ), WCHAR_T_CHARCODEFORMAT ) ;
	...
	else
	{
		for(;;)
		{
			...
			ConvString( FileName, DX_CHARCODEFORMAT_UTF8, ( char * )FileNameW, sizeof( FileNameW ), WCHAR_T_CHARCODEFORMAT ) ;
