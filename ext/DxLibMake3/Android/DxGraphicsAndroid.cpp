...
extern int Graphics_Android_Shader_Normal3DDraw_Setup( void )
{
	...
	if( GSYS.DrawSetting.ShadowMapDraw )
	{
		...
		FS           =  &SB3D->Base3D_ShadowMap_Normal_FS     [ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		FSAddress    = &SCB3D->Base3D_ShadowMap_Normal_FS_Code[ IgnoreTextureAlpha ][ AlphaTestMode ] ;

		Shader       = &SB3D->Base3D_ShadowMap_Normal_Shader[ BumpMap ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
	}
	else
	if( GANDR.Device.State.Lighting )
	{
		...
		FS_PL        =  &SB3D->Base3D_PixelLighting_Normal_FS     [ ShadowMap ][ LightIndex84 ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		FSAddress_PL = &SCB3D->Base3D_PixelLighting_Normal_FS_Code[ ShadowMap ][ LightIndex84 ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;

		Shader_PL    =  &SB3D->Base3D_PixelLighting_Normal_Shader[ ShadowMap ][ FogType ][ LightIndex84 ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		...
		FS           =  &SB3D->Base3D_VertexLighting_Normal_FS     [ ShadowMap ][ LightIndex10 ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		FSAddress    = &SCB3D->Base3D_VertexLighting_Normal_FS_Code[ ShadowMap ][ LightIndex10 ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;

		Shader       =  &SB3D->Base3D_VertexLighting_Normal_Shader[ ShadowMap ][ FogType ][ LightIndex20 ][ LightIndex10 ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
	}
	else
	{
		// ライティングなし描画
		...
		FS           =  &SB3D->Base3D_NoLighting_Normal_FS     [ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
		FSAddress    = &SCB3D->Base3D_NoLighting_Normal_FS_Code[ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;

		Shader       =  &SB3D->Base3D_NoLighting_Normal_Shader[ FogType ][ RenderType ][ GANDR.Device.DrawSetting.IgnoreGraphColorFlag ? 1 : 0 ][ IgnoreTextureAlpha ][ AlphaTestMode ] ;
	}
...
extern	int		Graphics_Android_SetupSubBackBuffer( void )
{
	...
	glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, GANDR.Device.Screen.SubBackBufferTextureSizeX, GANDR.Device.Screen.SubBackBufferTextureSizeY, 0, GL_RGB, GL_UNSIGNED_BYTE, 0 ) ;
	...
	// レンダーバッファの幅と高さを指定します。
	glRenderbufferStorage( GL_RENDERBUFFER, GL_DEPTH_COMPONENT16, GANDR.Device.Screen.SubBackBufferTextureSizeX, GANDR.Device.Screen.SubBackBufferTextureSizeY ) ;
...
)
{
	...
	// StretchRect用の設定を行う
	{
		...
		Graphics_Android_DeviceState_SetBlendMode( AlphaBlend ? DX_BLENDMODE_ALPHA : DX_BLENDMODE_NOBLEND ) ;
	...
	{
		// ブレンドモードを変更
		Graphics_Android_DeviceState_SetBlendMode( BlendMode ) ;
...
extern	int		Graphics_Android_Device_Create( void )
{
	...
	// バージョンやベンダー名を取得
	{
		...
		if( Version != NULL )
		{
			...
			ConvString( ( char * )Version, DX_CHARCODEFORMAT_ASCII, GANDR.Device.Caps.Version, sizeof( TCHAR ) * ( Length + 1 ), _TCHARCODEFORMAT ) ;
			if( Length * 4 + 2 < sizeof( TempStr ) )
			{
				ConvString( ( char * )Version, DX_CHARCODEFORMAT_ASCII, TempStr, sizeof( TempStr ), DX_CHARCODEFORMAT_UTF16LE ) ;
		...
		if( Renderer != NULL )
		{
			...
			ConvString( ( char * )Renderer, DX_CHARCODEFORMAT_ASCII, GANDR.Device.Caps.Renderer, sizeof( TCHAR ) * ( Length + 1 ), _TCHARCODEFORMAT ) ;
			if( Length * 4 + 2 < sizeof( TempStr ) )
			{
				ConvString( ( char * )Renderer, DX_CHARCODEFORMAT_ASCII, TempStr, sizeof( TempStr ), DX_CHARCODEFORMAT_UTF16LE ) ;
		...
		if( Vendor != NULL )
		{
			...
			ConvString( ( char * )Vendor, DX_CHARCODEFORMAT_ASCII, GANDR.Device.Caps.Vendor, sizeof( TCHAR ) * ( Length + 1 ), _TCHARCODEFORMAT ) ;
			if( Length * 4 + 2 < sizeof( TempStr ) )
			{
				ConvString( ( char * )Vendor, DX_CHARCODEFORMAT_ASCII, TempStr, sizeof( TempStr ), DX_CHARCODEFORMAT_UTF16LE ) ;
		...
		if( Shading_Language_Version != NULL )
		{
			...
			ConvString( ( char * )Shading_Language_Version, DX_CHARCODEFORMAT_ASCII, GANDR.Device.Caps.Shading_Language_Version, sizeof( TCHAR ) * ( Length + 1 ), _TCHARCODEFORMAT ) ;
			if( Length * 4 + 2 < sizeof( TempStr ) )
			{
				ConvString( ( char * )Shading_Language_Version, DX_CHARCODEFORMAT_ASCII, TempStr, sizeof( TempStr ), DX_CHARCODEFORMAT_UTF16LE ) ;
	...
	// Extensions 解析
	{
		...
		for(;;)
		{
			...
			ConvString( TempBuffer, DX_CHARCODEFORMAT_ASCII, GANDR.Device.Caps.ExtensionNames[ l ], sizeof( TCHAR ) * ( k + 1 ), _TCHARCODEFORMAT ) ;
			l ++ ;
			DXFREE( TempBuffer ) ;
			TempBuffer = NULL ;
...
//		) ;
		...
		// ブレンドモード設定
		Graphics_Android_DeviceState_SetBlendMode( DX_BLENDMODE_NOBLEND ) ;
...
//	Graphics_Android_DeviceState_SetShadeMode             ( GANDR.Device.State.ShadeMode           ) ;
	...
	Graphics_Android_DeviceState_SetViewport              ( &Viewport ) ;
	Graphics_Android_DeviceState_SetBackgroundColor       ( GANDR.Device.State.BackgroundColor.r, GANDR.Device.State.BackgroundColor.g, GANDR.Device.State.BackgroundColor.b ) ;
...
extern int Graphics_Android_DeviceState_SetSampleFilterMode( GLenum Filter, int Sampler )
{
	...
	if( Sampler < 0 )
	{
		...
		if( GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			for( i = 0 ; i < USE_TEXTURESTAGE_NUM ; i ++ )
			{
				if( GANDR.Device.State.TexMagFilter[ i ] != MagFilter ||
		...
		for( i = 0 ; i < USE_TEXTURESTAGE_NUM ; i ++ )
		{
			...
			if( GANDR.Device.State.SetTexture[ i ] != NULL )
			{
				...
				{
					GANDR.Device.State.SetTexture[ i ]->MinFilter = GANDR.Device.State.TexMinFilter[ i ] ;
	...
	else
	{
		...
		if( GANDR.Device.State.TexMagFilter[ Sampler ] == MagFilter &&
		...
		if( GANDR.Device.State.SetTexture[ Sampler ] != NULL )
		{
			...
			{
				GANDR.Device.State.SetTexture[ Sampler ]->MinFilter = GANDR.Device.State.TexMinFilter[ Sampler ] ;
...
extern int  Graphics_Android_DeviceState_SetTextureAddress( int AddressMode, int Sampler )
{
	...
	if( Sampler < 0 )
	{
		if( GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			for( i = 0 ; i < USE_TEXTURESTAGE_NUM ; i ++ )
			{
				if( WrapMode != GANDR.Device.State.TexAddressModeU[ i ] ||
					WrapMode != GANDR.Device.State.TexAddressModeV[ i ] ||
					WrapMode != GANDR.Device.State.TexAddressModeW[ i ] )
				{
					break ;
				}
			}
	...
	else
	{
		...
		if( WrapMode == GANDR.Device.State.TexAddressModeU[ Sampler ] &&
			WrapMode == GANDR.Device.State.TexAddressModeV[ Sampler ] &&
			WrapMode == GANDR.Device.State.TexAddressModeW[ Sampler ] &&
			GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			return 0 ;
		}
...
extern int Graphics_Android_DeviceState_SetTextureAddressV( int AddressMode, int Sampler )
{
	...
	if( Sampler == -1 )
	{
		if( GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			for( i = 0 ; i < USE_TEXTURESTAGE_NUM ; i ++ )
			{
				if( WrapMode != GANDR.Device.State.TexAddressModeV[ i ] )
				{
					break ;
				}
			}
	...
	else
	{
		...
		if( WrapMode == GANDR.Device.State.TexAddressModeV[ Sampler ] &&
			GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			return 0 ;
		}
...
extern int Graphics_Android_DeviceState_SetTextureAddressW( int AddressMode, int Sampler )
{
	...
	if( Sampler == -1 )
	{
		if( GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			for( i = 0 ; i < USE_TEXTURESTAGE_NUM ; i ++ )
			{
				if( WrapMode != GANDR.Device.State.TexAddressModeW[ i ] )
				{
					break ;
				}
			}
	...
	else
	{
		...
		if( WrapMode == GANDR.Device.State.TexAddressModeW[ Sampler ] &&
			GANDR.Device.DrawSetting.CancelSettingEqualCheck == FALSE )
		{
			return 0 ;
		}
...
extern	int		Graphics_Android_DeviceState_SetDrawMode( int DrawMode )
{
	...
	case DX_DRAWMODE_BILINEAR :
		// バイリニア描画
		Graphics_Android_DeviceState_SetSampleFilterMode( GL_LINEAR, -1 ) ;
		break ;
...
// 背景色を設定する
extern	int		Graphics_Android_DeviceState_SetBackgroundColor( int Red, int Green, int Blue )
{
	float RedF ;
	float GreenF ;
	float BlueF ;
	...
	glClearColor( RedF, GreenF, BlueF, 0.0f ) ;
...
// 描画ブレンドモードのセット
extern	int		Graphics_Android_DeviceState_SetBlendMode( int BlendMode )
{
...
extern int Graphics_Android_DeviceState_SetTexture( int SlotIndex, GRAPHICS_ANDROID_TEXTURE *Texture )
{
	...
	else
	{
		...
		if( GANDR.Device.DrawSetting.CancelSettingEqualCheck )
		{
			...
			Texture->MinFilter = GANDR.Device.State.TexMinFilter[ SlotIndex ] ;
...
extern	int		Graphics_Android_DeviceState_NormalDrawSetup( void )
{
	...
	Graphics_Android_DeviceState_SetBlendMode( NextBlendMode ) ;
...
//	Graphics_DrawSetting_SetUse2DProjectionMatrix( ( ParamFlag & DX_ANDROID_DRAWPREP_3D ) == 0 ? TRUE : FALSE ) ;
	...
	if( ParamFlag & DX_ANDROID_DRAWPREP_TEXADDRESS )
	{
		for( i = 0; i < USE_TEXTURESTAGE_NUM; i++ )
		{
			...
			{
				if( GANDR.Device.State.TexAddressModeU[ i ] != g_DXTexAddrModeToGLES2WrapMode[ GSYS.DrawSetting.TexAddressModeU[ i ] ] ||
					GANDR.Device.State.TexAddressModeV[ i ] != g_DXTexAddrModeToGLES2WrapMode[ GSYS.DrawSetting.TexAddressModeV[ i ] ] )
				{
			...
			else
			{
				...
				if( GANDR.Device.State.TexAddressModeV[ i ] != g_DXTexAddrModeToGLES2WrapMode[ GSYS.DrawSetting.TexAddressModeV[ i ] ] )
				{
	...
	else
	{
		for( i = 0; i < 4; i++ )
		{
			if( GANDR.Device.State.TexAddressModeU[ i ] != GL_CLAMP_TO_EDGE ||
				GANDR.Device.State.TexAddressModeV[ i ] != GL_CLAMP_TO_EDGE )
			{
				Graphics_Android_DeviceState_SetTextureAddress( GL_CLAMP_TO_EDGE, i ) ;
			}
		}
	}
...
extern	int		Graphics_Hardware_DrawRotaGraphFast_PF( int x, int y, float xf, float yf, float ExtendRate, float Angle, IMAGEDATA *Image, IMAGEDATA *BlendImage, int TransFlag, int TurnFlag, int IntFlag )
{
	VERTEX_2D *DrawVert ;
	VERTEX_BLENDTEX_2D *DrawVertB ;
	DWORD DiffuseColor ;
...
{
	Graphics_Android_DeviceState_SetBackgroundColor( Red, Green, Blue ) ;
...
{
	// 描画対象の深度バッファを初期化
//	AndroidDeviceContext_ClearDepthStencilView_ASync( GANDR.Device.State.TargetDepthDSV, D_ANDROID_CLEAR_DEPTH, 1.0f, 0 ) ;
...
{
	glBufferSubData( VertexBuffer->PF->VertexBuffer, SetIndex * VertexBuffer->UnitSize, VertexNum * VertexBuffer->UnitSize, VertexData ) ;
...
{
	glBufferSubData( IndexBuffer->PF->IndexBuffer, SetIndex * IndexBuffer->UnitSize, IndexNum * IndexBuffer->UnitSize, IndexData ) ;
